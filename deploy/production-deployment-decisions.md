...
2026-09-07 决策更新：所有购买统一读取后台微信支付配置，不再设置模块支付开关或支付部署阶段。方案上架、用户市场内容开放仍独立管理。下文 2026-07-24 首发记录仅代表历史状态；本次代码变更是否已发布须以具体发布记录为准。
...
2026-08-25 核心灰度优先级更新：知识星球会员首先通过派聪明扫码注册/登录和 `/vip/claim` 开通 VIP，再完成简历编辑、排版导出与 AI 分析。只读生产核查已经确认登录二维码能够生成、邀请码页面能够打开，但尚未用新微信身份完成扫码回调和后续真实操作。发布保持扫码登录启用；先受控部署，再按 `deploy/checklists/planet-core-acceptance.md` 留存真实验收结果。2026-09-07 用户明确纠正：验收标记不再作为待验收版本的部署前置条件。
...
→ 确认本次改动的定向验证结果；仅在当前任务明确要求时执行全量测试
→ 在本地受控 Linux 环境生成发布制品
...
### Admin 提示词随发布同步（2026-09-08）

字段优化运行时只读取数据库中的 Admin 配置，不读取本地 YAML，也不回退到 Java 文案。V46 只初始化缺失的两套字段优化配置，保留已保存的 Admin 修改。

发布前先在本地 Admin 调整并保存提示词。`launch.sh build/deploy` 从本地 `.env` 指向的数据库导出两套字段优化和四套简历分析配置，固化为制品中的 `config/admin-ai-prompts.json`，包含在逐文件校验中。这是本次发布的数据快照，不是运行时模板；若本地配置缺失或数据库不可用，构建会停止。

部署入口校验并更新本次制品携带的发布控制脚本，旧脚本备份在 `bin/previous-controls/<版本>/`。激活前停止应用、备份线上提示词，事务内写入本次快照，再回读比较；成功后启动新版本。同步失败或应用验收失败时恢复发布前提示词，保留数据库整库备份。回滚同样恢复发布前的提示词。生产机需要 Python 3 和 MySQL 客户端。

只同步提示词内容、名称、说明和排序，不同步账号、简历、AI 密钥或支付配置。发布写入的 `updated_by=0` 表示发布操作，不将本地管理员 ID 冒充为线上管理员。线上 Admin 后续编辑仍立即生效，服务重启不会重新导入；下一次明确部署时以当次本地 Admin 快照为准。
