...
| 人工精修 OSS | 首发总开关关闭；现有共享私有 Bucket 已配置 PaiResume 独立前缀、精确 CORS 和生命周期，待 RAM 最小权限核验后再开放 |
...
当前状态：PaiResume 侧接收和校验代码已经存在，但 paicoding 生产桥接 endpoint 仍返回 404，尚未完成对应桥接代码发布和双方真实参数配置。本次首发以“不影响已有服务”为先，不修改或重启 paicoding，因此：
...
后续把 paicoding 桥接作为独立发布，完成 endpoint、双向 HMAC、重放保护、未关注/已关注扫码和一次性换票真实验收后，才同时填写真实 AppID/独立密钥并启用开关。扫码登录是已知待办，但不再阻断本次邮箱登录首发。

## 四、人工精修不再接入第二座公众号桥

2026-07-24 已确认删除“关注沉默王二后第二次免费”的产品规则，避免用户在登录后还要再次扫码。人工精修统一为：
...
2. 人工精修由用户在浏览器选择最终确认的 PDF，再使用服务端签发的短期、单对象 POST Policy 直传私有阿里云 OSS；Policy 同时限制精确 key、与声明字节数完全相等且不超过 10 MiB 的文件大小、PDF 类型、SHA 元数据、私有 ACL 和 `AES256`。
...
人工精修总开关 `RESUME_REVIEW_ENABLED` 默认并首发保持 `false`：关闭时停止新验证码、新请求、上传授权/完成和精修邮件投递，前端隐藏入口，但保留历史只读、退款、支付回调和对账。此时生产预检与 Java 启动校验不要求人工精修 OSS、RAM 或私密收件箱。准备启用前必须用测试 PDF 验证短期上传策略、服务端核验、冻结对象、邮件附件、失败重试、staging 过期清理和保留期删除；不得用真实用户简历做自动化冒烟数据。Bucket 必须为私有，CORS 只允许生产来源的 `POST` 和必要请求头，staging 1 天清理、冻结对象按配置天数删除。应用 RAM 仅授予两个前缀的 `oss:GetObject`、`oss:PutObject`，不授予 `oss:DeleteObject`、Bucket 配置或列举权限；对象删除完全由独立生命周期规则负责。以上均由独立运维身份核验留证后，再把对应四个 `RESUME_REVIEW_OSS_*_CONFIRMED` 预检位全部改为 `true`；总开关打开时，任一确认位不是严格的 `true` 都必须阻断启动和发布。
...
- 业务开关：`DEPLOY_STAGE=free`、`PAYMENT_PROVIDER=disabled`、会员新单关闭、用户市场关闭、人工精修及 OSS 关闭、派聪明扫码桥关闭；Redis 使用 DB 1 和 `pairesume:prod:`。
...
- [x] 删除第二座公众号桥、第二个二维码和关注换免费规则；人工精修改为首次免费、第二次起付费。
- [x] 本地代码和发布包边界已改为浏览器普通导出、人工精修私有 OSS 直传与服务端内存读件，不再依赖 Node/PDF worker。
- [ ] 共享私有 OSS 的 ACL、精确 POST CORS 和两条生命周期已真实核验；RAM 最小权限仍未证实，因此人工精修总开关保持 `false`。
