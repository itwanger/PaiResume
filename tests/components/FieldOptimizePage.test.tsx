...
describe('field optimization ownership', () => {
  ...
  it('量化草稿先补充真实数据，再允许采纳', async () => {
    const user = userEvent.setup()
    api.getLatestFieldOptimizeRecord.mockResolvedValue(envelope({ status: 'completed', candidates: ['审批耗时由【待补充：优化前耗时（秒）】降至【待补充：优化后耗时（秒）】'], candidateTags: [['quantified']] }))
    mount()
    await screen.findByText('待补数据')
    expect(screen.queryByRole('button', { name: '采纳这个版本' })).not.toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: '补充量化数据' }))
    const input = screen.getByRole('textbox', { name: '版本 1内容' })
    await user.clear(input)
    await user.type(input, '同等任务集下审批耗时由 20 秒降至 12 秒。')
    expect(screen.queryByText('待补数据')).not.toBeInTheDocument()
    expect(screen.getByRole('button', { name: '采纳这个版本' })).toBeEnabled()
    expect(store.updateModuleContent).not.toHaveBeenCalled()
  })
