...
public class FeedbackSubmissionCreateDTO {
    ...
    @Size(max = 64, message = "展示名称不能超过 64 个字符")
    private String displayName;

    @NotBlank(message = "学校或公司不能为空")
    @Size(max = 128, message = "学校或公司不能超过 128 个字符")
    private String schoolOrCompany;

    @NotBlank(message = "目标岗位不能为空")
    @Size(max = 128, message = "目标岗位不能超过 128 个字符")
    ...
    @Size(max = 2000, message = "评价内容不能超过 2000 个字符")
    private String testimonialText;

    @Size(max = 5000, message = "需求建议不能超过 5000 个字符")
    private String desiredFeatures;

    @Size(max = 5000, message = "问题反馈不能超过 5000 个字符")
